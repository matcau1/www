 ...
 * @since 1.0.0
 */
class WlFeaturesBoxV4 extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return __( 'WL Features Box V4', 'wl-elementor' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'section_wl-features-box-v4',
			[
				'label' => esc_html__( 'WL Features Box V4 Area', 'wl-elementor' ),
			]	
		);
		...
		$this->add_control(
			...
			[
				...
				'fields' => [
					...
					[
						'name'    => 'feature_icon',
						'label'   => esc_html__( 'Feature Image Icon (Recommend Size 110 * 110)', 'wl-elementor' ),
	...
	<div class="<?php echo wp_kses_post($settings['custom_class']); ?>">
		...
		<?php foreach ( $settings['features_items'] as $item ) : ?>
			...
			<div class="<?php echo wp_kses_post($item['custom_column_class']); ?>">
	            <div class="d-flex justify-content-center <?php echo wp_kses_post($item['custom_border']); ?>">
	                <div class="flex-shrink-0 me-3">
	                	<?php if ( '' !== $item['feature_icon']['url'] ) : ?>
	                    	<img src="<?php echo wp_kses_post($item['feature_icon']['url']); ?>" alt="<?php echo wp_kses_post($item['features_main_alt_tag']); ?>" title="<?php echo wp_kses_post($item['features_main_title_tag']); ?>" class="w-55px">
	                    <?php endif; ?>
	                </div>
	                <div class="flex-grow-1">
	                	<?php if ( '' !== $item['features_title'] ) : ?>
	                    	<h4 class="h5 mb-1"><?php echo wp_kses_post($item['features_title']); ?></h4>
	                    <?php endif; ?>

	                    <?php if ( '' !== $item['features_content'] ) : ?>
	                    	<span class="text-muted"><?php echo wp_kses_post($item['features_content']); ?></span>
	                    <?php endif; ?>
	                </div>
	            </div>
	        </div>
