 ...
 * @since 1.0.0
 */
class WlClientsV1 extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return __( 'WL Clients V1', 'wl-elementor' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'section_wl-clients-v1',
			[
				'label' => esc_html__( 'WL Clients V1 Area', 'wl-elementor' ),
			]	
		);
		...
		$this->add_control(
			...
			[
				...
				'fields' => [
					...
					[
						'name'    => 'client_image',
						'label'   => esc_html__( 'Client Main Image (Recommend Size 127 * 49)', 'wl-elementor' ),
	        ...
	        <div class="<?php echo wp_kses_post($item['custom_column_class']); ?> wow fadeIn" data-wow-delay="<?php echo wp_kses_post($item['animation_delay']); ?>ms">
	            <div class="text-center client-img">
	            	<?php if ( '' !== $item['client_image']['url'] ) : ?>
	                	<img src="<?php echo wp_kses_post($item['client_image']['url']); ?>" alt="<?php echo wp_kses_post($item['client_image_alt_tag']); ?>" title="<?php echo wp_kses_post($item['client_image_title_tag']); ?>">
	                <?php endif; ?>
	            </div>
	        </div>
