 ...
 * @since 1.0.0
 */
class WlProcessV3 extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return __( 'WL Process V3', 'wl-elementor' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'section_wl-process-v3',
			[
				'label' => esc_html__( 'WL Process V3 Area', 'wl-elementor' ),
			]	
		);
    ...
    <div class="<?php echo wp_kses_post($settings['custom_class']); ?>">
    	...
    	<?php foreach ( $settings['process_items'] as $item ) : ?>
            ...
            <div class="<?php echo wp_kses_post($item['custom_column_class']); ?> wow fadeInRight" data-wow-delay="<?php echo wp_kses_post($item['animation_delay']); ?>ms">
                <div class="text-center process-inner px-xxl-1-9">
                	<?php if ( '' !== $item['process_no'] ) : ?>
                    	<span><?php echo wp_kses_post($item['process_no']); ?></span>
                    <?php endif; ?>

                    <?php if ( '' !== $item['process_title'] ) : ?>
                    	<h3 class="h4 mb-3 text-white"><?php echo wp_kses_post($item['process_title']); ?></h3>
                    <?php endif; ?>

                    <?php if ( '' !== $item['process_content'] ) : ?>
                    	<p class="text-white opacity6 mb-0"><?php echo wp_kses_post($item['process_content']); ?></p>
                    <?php endif; ?>
                </div>
            </div>
