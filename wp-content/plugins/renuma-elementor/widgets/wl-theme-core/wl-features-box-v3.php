 ...
 * @since 1.0.0
 */
class WlFeaturesBoxV3 extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return __( 'WL Features Box V3', 'wl-elementor' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'section_wl-features-box-v3',
			[
				'label' => esc_html__( 'WL Features Box V3 Area', 'wl-elementor' ),
			]	
		);
			...
			[
				...
				'fields' => [
					...
					[
						...
						'label'       => esc_html__( 'Custom Background', 'wl-elementor' ),
						'type'        => Controls_Manager::TEXTAREA,
						'dynamic'     => [ 'active' => true ],
						'default'     => esc_html__( 'Custom Background' , 'wl-elementor' ),
						'label_block' => true,
					],
					[
						'name'    => 'feature_icon',
						'label'   => esc_html__( 'Feature Image Icon (Recommend Size 120 * 120)', 'wl-elementor' ),
    ...
    <div class="<?php echo wp_kses_post($settings['custom_class']); ?>">
    	...
    	<?php foreach ( $settings['features_items'] as $item ) : ?>
	        ...
	        <div class="<?php echo wp_kses_post($item['custom_column_class']); ?> wow fadeInUp" data-wow-delay="<?php echo wp_kses_post($item['animation_delay']); ?>ms">
	            <div class="card card-style23 <?php echo wp_kses_post($item['custom_background']); ?>">
	                <div class="card-body p-1-6 p-xxl-2-6">
	                    <div class="mb-5">
	                    	<?php if ( '' !== $item['feature_icon']['url'] ) : ?>
	                        	<img src="<?php echo wp_kses_post($item['feature_icon']['url']); ?>" alt="<?php echo wp_kses_post($item['features_main_alt_tag']); ?>" title="<?php echo wp_kses_post($item['features_main_title_tag']); ?>" class="w-60px">
	                        <?php endif; ?>
	                    </div>
	                    <?php if ( '' !== $item['features_title'] ) : ?>
	                    	<?php echo wp_kses_post($item['features_title']); ?>
	                    <?php endif; ?>

	                    <?php if ( '' !== $item['features_content'] ) : ?>
	                    	<?php echo wp_kses_post($item['features_content']); ?>
	                    <?php endif; ?>
	                </div>
	            </div>
	        </div>
