 ...
 * @since 1.0.0
 */
class WlServiceV6 extends \Elementor\Widget_Base {
	...
	public function get_title() {
		return __( 'WL Service V6', 'wl-elementor' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'section_wl-service-v6',
			[
				'label' => esc_html__( 'Wl Service V6 Area', 'wl-elementor' ),
			]	
		);
			...
			[
				...
				'fields' => [
					[
						...
						'label'   => esc_html__( 'Service Bg Image (Recommend Size 551 * 420)', 'wl-elementor' ),
    ...
    <div class="<?php echo wp_kses_post($settings['slider_class']); ?>">
    	...
    	<?php foreach ( $settings['service_items'] as $item ) : ?>
	        ...
	        <div class="card card-style21 border-0">
	            <div class="service-card bg-img cover-background" <?php if ( '' !== $item['service_bg_image']['url'] ) : ?> data-background="<?php echo wp_kses_post($item['service_bg_image']['url']); ?>" <?php endif; ?>>
	                <div class="service-details">
	                	<?php if ( '' !== $item['service_title'] ) : ?>
	                    	<h3 class="mb-3 h5"><a href="<?php echo wp_kses_post($item['service_detail_page_link']); ?>"><?php echo wp_kses_post($item['service_title']); ?></a></h3>
	                    <?php endif; ?>

	                    <?php if ( '' !== $item['service_content'] ) : ?>
	                    	<p class="mb-3"><?php echo wp_kses_post($item['service_content']); ?></p>
	                    <?php endif; ?>

	                    <?php if ( '' !== $item['service_detail_page_text'] ) : ?>
	                    	<a href="<?php echo wp_kses_post($item['service_detail_page_link']); ?>"><?php echo wp_kses_post($item['service_detail_page_text']); ?></a>
	                    <?php endif; ?>
	                </div>
	            </div>
	        </div>
