 ...
 * @since 1.0.0
 */
class WlPortfolioCarouselV2 extends \Elementor\Widget_Base {
	...
	public function get_name() {
		return 'wl-portfolio-carousel-v2';
	}
	...
	public function get_title() {
		return __( 'Wl Portfolio Carousel V2', 'wl-elementor' );
	}
	...
	protected function register_controls() {
		$this->start_controls_section(
			'section_wl-portfolio-carousel-v2',
			[
				'label' => esc_html__( 'Wl Portfolio Carousel V2 Area', 'wl-elementor' ),
			]	
		);
		...
		$this->add_control(
			...
			[
				...
				'fields' => [
					...
					[
						'name'    => 'portfolio_img',
						'label'   => esc_html__( 'Portfolio Image (Recommend Size 601 * 800)', 'wl-elementor' ),
	        ...
	        <div class="portfolio-style2">
	            <div class="portfolio-item-link">
	            	<?php if ( '' !== $item['arrow_icon'] ) : ?>
	                	<a href="<?php echo wp_kses_post($item['portfolio_detail_page_link']); ?>"><span class="<?php echo wp_kses_post($item['arrow_icon']); ?>"></span></a>
	                <?php endif; ?>

	                <?php if ( '' !== $item['portfolio_img']['url'] ) : ?>
	                	<div class="portfolio-item-media rounded"><img src="<?php echo wp_kses_post($item['portfolio_img']['url']); ?>" alt="<?php echo wp_kses_post($item['portfolio_image_alt_tag']); ?>" title="<?php echo wp_kses_post($item['portfolio_image_title_tag']); ?>"></div>
	                <?php endif; ?>
	                <span class="portfolio-item-content">
	                	<?php if ( '' !== $item['portfolio_catagory'] ) : ?>
	                    	<span class="text-white display-28 mb-2 d-block"><?php echo wp_kses_post($item['portfolio_catagory']); ?></span>
	                    <?php endif; ?>

	                    <?php if ( '' !== $item['portfolio_title'] ) : ?>
	                    	<h4 class="mb-0"><a href="<?php echo wp_kses_post($item['portfolio_detail_page_link']); ?>" class="text-white text-secondary-hover"><?php echo wp_kses_post($item['portfolio_title']); ?></a></h4>
	                    <?php endif; ?>
	                </span>
	            </div>
	        </div>
